...
int mptHeuristic(const vector<vector<char>> &current, const vector<vector<char>> &goal) {
...
void solvePuzzle(const vector<vector<char>> &initial, const vector<vector<char>> &goal) {
    ...
    State initialState = {initial, 0, 0};
    initialState.cost_h2 = mptHeuristic(initial, goal);
    ...
    while (!pq.empty() && step < maxSteps) {
        ...
        for (const auto &move : moves) {
            ...
            if (newI >= 0 && newI < N && newJ >= 0 && newJ < N) {
                ...
                nextState.cost_h2 = mptHeuristic(nextState.puzzle, goal);
                pq.push(nextState);
            }	}	}
